def da_boas_vindas
	puts
    puts "        P  /_\  P                              "
    puts "       /_\_|_|_/_\                             "
    puts "   n_n | ||. .|| | n_n         Bem vindo ao    "
    puts "   |_|_|nnnn nnnn|_|_|     Jogo de Adivinhação!"
    puts "  |' '  |  |_|  |'  ' |                        "
    puts "  |_____| ' _ ' |_____|                        " 
    puts "        \__|_|__/                              "
    puts
    puts "Qual é o seu nome?"
	nome = gets.strip
	puts "\n\n\n" 
	puts "Começamos o jogo, #{nome}"
	nome
end
def pede_dificuldade
	puts "Qual o nível de dificuldade?"
    puts "(1) Muito fácil (2) Fácil (3) Normal (4) Difícil (5) Impossível"
    puts "Escolha: "
	dificuldade = gets.to_i
end	

def sorteia_numero_secreto(dificuldade)
	case dificuldade
	when 1
		maximo = 30
	when 2
		maximo = 60
	when 3
		maximo = 100
	when 4
		maximo = 150
	when 5
		maximo = 200
	end	
			
	puts "Escolhendo um número secreto entre 0 e #{maximo}..."
	sorteado = rand(maximo) + 1
	puts "Escolhendo... que tal adivinhar hoje nosso núme secreto?"
	sorteado #return sorteado
end

def pede_um_numero(chutes, tentativa, limite)
	puts "\n\n\n"
	puts "Tentativa " + tentativa.to_s + " de " + limite.to_s
	puts "Chutes até agora: #{chutes}"
	puts "Digite o número: "
	chute = gets.strip
	puts "Será que acertou? Você chutou #{chutes}"
	chute.to_i
end

def verifica_se_acertou(num_secreto, chute)
	acertou = chute.to_i == num_secreto

	if (acertou)
		ganhou
		return true
	end

	maior = num_secreto > chute.to_i
	if maior
		puts "O número secreto é maior!"
	else
		puts "O número secreto é menor!"
	end
	false
end

def joga(nome, dificuldade)
	num_secreto = sorteia_numero_secreto(dificuldade)

	pontos_ate_agora = 1000
	max_tentativas = 5
	chutes = []

	for i in 1..max_tentativas
		chute = pede_um_numero chutes, i, max_tentativas
		chutes << chute
		
		if nome == "Obama"
			ganhou
			break
		end

		pontos_a_perder = (chute - num_secreto).abs / 2.0
		pontos_ate_agora -= pontos_a_perder

		break if verifica_se_acertou num_secreto, chute

	end
	puts "Você ganhou #{pontos_ate_agora} pontos."
end

def nao_quer_jogar?
	puts "Deseja jogar novamente? (S/n)"
	quero_jogar = gets.strip
	nao_quer_jogar = quero_jogar.upcase == "N"
end

def ganhou
    puts
    puts "             OOOOOOOOOOO               "
    puts "         OOOOOOOOOOOOOOOOOOO           "
    puts "      OOOOOO  OOOOOOOOO  OOOOOO        "
    puts "    OOOOOO      OOOOO      OOOOOO      "
    puts "  OOOOOOOO  #   OOOOO  #   OOOOOOOO    "
    puts " OOOOOOOOOO    OOOOOOO    OOOOOOOOOO   "
    puts "OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO  "
    puts "OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO  "
    puts "OOOO  OOOOOOOOOOOOOOOOOOOOOOOOO  OOOO  "
    puts " OOOO  OOOOOOOOOOOOOOOOOOOOOOO  OOOO   "
    puts "  OOOO   OOOOOOOOOOOOOOOOOOOO  OOOO    "
    puts "    OOOOO   OOOOOOOOOOOOOOO   OOOO     "
    puts "      OOOOOO   OOOOOOOOO   OOOOOO      "
    puts "         OOOOOO         OOOOOO         "
    puts "             OOOOOOOOOOOO              "
    puts
    puts "         Acertou mizeravi!! :)         "
    puts
end

nome = da_boas_vindas
dificuldade = pede_dificuldade

loop do
	joga nome, dificuldade
	if nao_quer_jogar?
		break
	end
end